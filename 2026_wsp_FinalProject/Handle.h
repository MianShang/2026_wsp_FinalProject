...
//윈도우 OS와 정보 교환하는 가장 기초적인 자료형 HANDLE을 보관하고, 사용할 수 있도록 만들어 주는 클래스를 선언

class Handle
{
	// 접근 권한 설정
	// C++에서는 별도로 설정하지 않으면 private 권한
	// JAVA에서는 별도로 설정하지 않으면 public 권한
private:
	// 내부에서 관리하는 실제 핸들 객체
	HANDLE m_handle;	// 우선 자료형 선언하기
...
public:
	...
	// 멤버 변수인 HANDLE을 자동으로 해제하는 소멸자 선언. 반환값, 인수 없음. 그러므로 오버로딩 불가
	~Handle();
